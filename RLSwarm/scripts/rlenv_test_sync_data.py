...
from RLSwarm.envs.deprecated.rl_swarm_env import RLSwarmEnv  # Use RLSwarmEnv instead of FullSwarmGymEnv
...
class DummyPolicy(nn.Module):
    ...
    def forward(self, tensordict: TensorDict):
        # The policy receives the full tensordict. Observations are in tensordict['agents'].
...
def test_sync_collector_rl_swarm():
    """Test RLSwarmEnv compatibility with SyncDataCollector using a dummy policy."""
    print("Starting SyncDataCollector test for RLSwarmEnv with dummy policy...")
    ...
    # Create RLSwarmEnv (already wrapped with GymWrapper)
    rlenv = RLSwarmEnv(config=config)
    env = GymWrapper(rlenv, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    policy = TensorDictModule(
        ...
        in_keys=["agents"],  # Specify in_keys to match observation keys under 'agents'
    ...
    for i, data in enumerate(collector):
        ...
        # Access observations inside the 'agents' nested tensordict
        print(f"Sample depth_image shape: {data['agents']['depth_image'].shape}")
        print(f"Sample position shape: {data['agents']['position'].shape}")
